...
import { CreateLollyMutation } from "../API";
...
const CreatePage = () => {
  ...
  const handleFreezeKulfy = async (
    ...
    try {
      const { data: lolly } = (await API.graphql(
        graphqlOperation(mutations.createLolly, {
          colorTop,
          colorBottom,
          colorMiddle,
          to: values.to,
      ...
      //   },
      // });
      console.log("Create Kulfi Successfully", lolly);
      await navigate(`/dynamiclolly/${lolly.createLolly.id}`);
    } catch (error) {
